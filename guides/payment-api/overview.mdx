...
The Payment API is the fastest way to start collecting bank-to-bank payments with Quidkey. Create a payment from your backend, send your buyer to a bank to approve it, and receive a webhook the moment it settles. One set of credentials, one authentication flow, and one webhook infrastructure power every integration path.
...
<Note>
**Amounts are always integer minor units.** `1000` = £10.00, `2550` = €25.50. This matches Stripe's format exactly, across every endpoint in the Payment API. See [Amounts & Currencies](/guides/payment-api/concepts/amounts-and-currencies).
</Note>

## Base URL

All Payment API requests go to a single base URL:
...
Every path uses the same OAuth credentials, the same integer minor-unit amounts, and the same webhooks. They differ only in where the buyer completes the payment and how much frontend code you write.
...
<Step title="Webhook">
  Quidkey sends a [webhook](/guides/payment-api/concepts/webhooks) to your backend with the final result. **The webhook is the source of truth**, not the browser redirect. Fulfil orders only when you receive `quidkey.payment_request.succeeded`.
</Step>
</Steps>
