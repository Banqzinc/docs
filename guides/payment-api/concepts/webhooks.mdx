...
| Field | Description |
|-------|-------------|
| `id` | Unique event ID (`evt_...`). **Use this to de-duplicate** — see [below](#delivery--de-duplication). |
| `object` | Always `event`. |
...
| `data.object.amount` | Amount in **integer minor units** (see [Amounts & Currencies](/guides/payment-api/concepts/amounts-and-currencies)). |
...
|------------|---------------|-------|
| `quidkey.payment_request.succeeded` | Payment is `COMPLETED` and funds `RECEIVED` | Includes the `fees` object. Fulfil the order here. |
...
```bash
curl 'https://core.quidkey.com/api/v1/merchants/MERCHANT_ID/payment-requests/PAYMENT_REQUEST_ID/status' \
  -H 'Authorization: Bearer YOUR_ACCESS_TOKEN'
```
