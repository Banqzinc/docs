...
| `error.errors` | Present on validation failures (`400`). An array of per-field problems. |
...
| `400` | Validation error | Malformed or invalid input. Field-level details in `errors[]`. |
...
A `400` indicates the request body or parameters failed validation. The `errors[]` array pinpoints each offending field, so you can surface precise feedback.
...
if (!body.success) {
  const { code, message, errors } = body.error;
