...
<Note>
**Amounts are integer minor units.** `2550` = £25.50, `1000` = €10.00. The same format used across the Payment API and by Stripe. See [Amounts & Currencies](/guides/payment-api/concepts/amounts-and-currencies).
</Note>
...
|-------|------|----------|-------------|
| `merchant_id` | string | Yes | UUID of the merchant collecting the payment |
| `customer.name` | string | Yes | Buyer's full name |
| `customer.email` | string | Yes | Buyer's email address |
| `customer.phone_number` | string | Yes | E.164 format phone number |
...
| `payment_reference` | string | No | Reference shown on the buyer's bank statement |
