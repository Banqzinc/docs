...
description: "Add Quidkey checkout alongside your existing Stripe integration"
icon: "code-merge"
---

This guide explains how to add the Quidkey bank payment option next to your existing Stripe Payment Element. The customer sees a single payment section where exactly one method (Stripe **or** Quidkey) can be selected at a time.

<Note>
**New to Quidkey?** Start with the [Quickstart](/quickstart) to get your first payment working in 10 minutes, then return here for production-ready implementation.
</Note>
...
- [ ] Server able to call external APIs (for Quidkey backend)
- [ ] Quidkey `client_id` and `client_secret` (sign up at [merchants.quidkey.com](https://merchants.quidkey.com) to get yours)
- [ ] HTTPS enabled on your checkout page

<Info>
**No Stripe changes required** - Your existing Stripe integration stays exactly as it is.
</Info>

<Tip>
**Estimated integration time:** 2-4 hours for production-ready setup including webhooks and error handling.
</Tip>
...
<Card title="Interactive API Playground" icon="flask" href="/api-reference/embedded/create-a-payment-request-and-return-a-payment_token-for-iframe-flow">
  Test creating a payment request in your browser before writing any code. Click "Send" to see a real response.
</Card>

<Info>
You can test every API call in the [API Reference](/api-reference) tab using the interactive playgrounds. No authentication required for testing.
</Info>
...
<Info>
You'll receive a `payment_token` (valid for 15 minutes) to embed in your iframe. See the [Create Payment Request API](/api-reference/embedded/create-a-payment-request-and-return-a-payment_token-for-iframe-flow) for the complete specification and response schema.
</Info>
...
<Step title="Generate the signing secret">
  ...
  <Tip>
  See the [Generate Webhook Secret API](/api-reference/webhook/generate-a-webhook-signing-secret-for-the-authenticated-merchant) for complete details and to test in the interactive playground.
  </Tip>
</Step>
